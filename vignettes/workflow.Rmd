...
ndf <- getsfm(peaklist, idelta=idelta, windows=windows, n=100, deltart=10, minn=6)
...
For integration with other Bioconductor packages, it is recommended to convert the feature table into a `SummarizedExperiment` object. This class is the standard container for rectangular data in Bioconductor and facilitates interoperability with tools like `DESeq2`, `limma`, or `SummarizedExperiment`-based metabolomics workflows.

```{r}
if (requireNamespace("SummarizedExperiment", quietly = TRUE)) {
  library(SummarizedExperiment)
  
  # Prepare the count matrix (features x samples)
  # Ensure the matrix contains only numeric intensity values
  # feature is a data.table from the previous step
  counts <- as.matrix(feature[, -1, with = FALSE])
  rownames(counts) <- feature$peakid
  
  # Create sample metadata (colData)
  # In a real study, this would include group information, time points, etc.
  # Here we create dummy metadata for demonstration
  colData <- DataFrame(
    SampleID = colnames(counts),
    Group = rep(c("Control", "Treatment"), length.out = ncol(counts)),
    row.names = colnames(counts)
  )
  
  # Create feature metadata (rowData)
  # Extract m/z and retention time from the peak ID
  peak_info <- do.call(rbind, strsplit(rownames(counts), "@"))
  rowData <- DataFrame(
    mz = as.numeric(peak_info[, 1]),
    rt = as.numeric(peak_info[, 2]),
